...
export const orderCar = createAsyncThunk("orderCar/Cars", async (payload) => {
    ...
    const NewOrderCar = await axios.post(`https://api-car-rental.binaracademy.org/customer/order/`, payload, config)
...
const initialState = {
    list_date : localStorage.getItem("updatedata") ? JSON.parse(localStorage.getItem("updatedata")) : {},
}
...
const createOrder = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder
        .addCase(orderCar.pending, (state, action) => {
            state.loading = true
            state.list_date = action.payload
        })
    }
